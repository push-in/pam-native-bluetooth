...
public final class BluetoothModule: NSObject, NativeModule, CBCentralManagerDelegate, CBPeripheralDelegate, @unchecked Sendable {
    ...
    public func invoke(method: String, payload: Data, completion: @escaping ModuleCompletion) {
        ...
        case "scan": try self.scan(v); try self.success(completion: completion)
        case "stopScan": self.stopScan(); try self.success(completion: completion)
        case "connect": try self.peripheral(v).delegate = self; self.central.connect(try self.peripheral(v)); try self.success(completion: completion)
        case "disconnect": self.central.cancelPeripheralConnection(try self.peripheral(v)); try self.success(completion: completion)
        case "read": let (_, c) = try self.characteristic(v); c.service?.peripheral.readValue(for: c); try self.success(completion: completion)
        case "write": let (p, c) = try self.characteristic(v); guard let data = Data(base64Encoded: try v.text("valueBase64")), data.count <= 512 else { throw BLEError.invalid }; p.writeValue(data, for: c, type: (try v.flag("response")) ? .withResponse : .withoutResponse); try self.success(completion: completion)
        case "subscribe": let (p, c) = try self.characteristic(v); p.setNotifyValue(try v.flag("enabled"), for: c); try self.success(completion: completion)
